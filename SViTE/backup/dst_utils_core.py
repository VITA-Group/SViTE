...
class Masking(object):
    ...
    def structure_init_new(self):
        ...
        for name_cur in self.atten_masks.keys():
            atten_dim = self.atten_masks[name_cur].shape[-1] # 768 for deit_base // linear weigth.size = (out_features, in_features)
            atten_dim0 = self.atten_masks[name_cur].shape[0] # out features 
            break 

        self.num_atten = layers_number * self.args.atten_head
        print('* Check if args.attan_head={} match the model'.format(self.args.atten_head))
        print('log: head numbers = {}'.format(self.num_atten))
        self.nonzeros_index = random.sample([i for i in range(self.num_atten)], int(self.num_atten * self.args.atten_density))
        print('log: ', self.nonzeros_index)
        muti_head_dim = int(atten_dim / self.args.atten_head) # 64 
        print(' multi_head_dimension = {}'.format(muti_head_dim))
        self.atten_mask_shape = (atten_dim0, muti_head_dim)
        self.atten_key_index = [k for k in self.atten_masks.keys()]
        print("-" * 100)
        ...
        for nonzero_idx in self.nonzeros_index:
            ...
            print('log: ', key, q_start, k_start, v_start, muti_head_dim)
            print('{} | {}/{}'.format(key, 
                self.atten_masks[key].sum().int().item(), 
                self.atten_masks[key].numel()))
        ...
        for name in self.other_masks.keys():
            ...
            # debug
            sparse_unit = ((self.other_masks[name] == 0).float().sum(1) == 0).float().sum(0).item()
            sparse_unit_fc2 = ((self.other_masks[name_fc2] == 0).float().sum(0) == 0).float().sum(1).item()
            print('log: ', name, name_fc2, sparse_unit, sparse_unit_fc2)
    ...
    def truncate_weights(self, pruning_type="unstructure", indicator_list=None):
        ...
        if pruning_type=="unstructure":
            ...
            # Death
            # step1: collect l1 norm indicator 
            l1_norm_mlp = {}
            number_remove = 0
            number_zeros = 0
            for name, weight in self.modules.named_parameters():
                name_cur = name + '_mask'
                # skip conv and last fc
                if name_cur in self.other_masks.keys():
                    number_remove += math.ceil(self.name2death_rate[name_cur]*self.name2nonzeros[name_cur])
                    number_zeros += self.name2zeros[name_cur]
            ...
            l1_indicator_list = []
            sum_l1_norm_dict = {}
            for key in l1_norm_mlp.keys():
                if 'mlp.fc1' in key:
                    name_fc2 = key.replace('fc1', 'fc2')
                    result = l1_norm_mlp[key] + l1_norm_mlp[name_fc2]
                    l1_indicator_list.append(result)
                    sum_l1_norm_dict[key] = result
                else: continue 
            
            l1_indicator_list = torch.cat(l1_indicator_list, dim=0)
            print(l1_indicator_list.size())
            thres = torch.sort(l1_indicator_list)[0][number_remove + number_zeros]
            print(thres)

            # step3: pruning MLP mask 
            for key in self.other_masks.keys():
                if 'mlp.fc1' in key:
                    name_fc2 = key.replace('fc1', 'fc2')
                    l1_norm = sum_l1_norm_dict[key]
                    enable_index = l1_norm > thres
            ...
            # step2: calculate threshold
            l1_indicator_list_gradient = []
            sum_l1_norm_dict_gradient = {}
            for key in l1_norm_mlp_gradient.keys():
                if 'mlp.fc1' in key:
                    name_fc2 = key.replace('fc1', 'fc2')
                    result_gradient = l1_norm_mlp_gradient[key] + l1_norm_mlp_gradient[name_fc2]
                    l1_indicator_list_gradient.append(result_gradient)
                    sum_l1_norm_dict_gradient[key] = result_gradient
                else: continue 
            l1_indicator_list_gradient = torch.cat(l1_indicator_list_gradient, dim=0)
            print(l1_indicator_list_gradient.size())
            thres_gradient = torch.sort(l1_indicator_list_gradient)[0][-number_remove]
            print(thres_gradient)
    
            # step 3 generate new mask 
            for key in self.other_masks.keys():
                if 'mlp.fc1' in key:
                    name_fc2 = key.replace('fc1', 'fc2')
                    l1_norm_gradient = sum_l1_norm_dict_gradient[key]
            ...
            for zero_idx in pruned_index:
                ...
                layer_index = int(zero_idx / self.args.atten_head)
                key = self.atten_key_index[layer_index]
                index_inside_layer = zero_idx % self.args.atten_head
                ...
                # q_mask
                self.atten_masks[key][q_start: q_start + muti_head_dim, :] = 0.
                # k_mask
                self.atten_masks[key][k_start: k_start + muti_head_dim, :] = 0.
                # v_mask
                self.atten_masks[key][v_start: v_start + muti_head_dim, :] = 0.
