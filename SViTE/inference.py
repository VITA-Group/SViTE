...
from engine import train_one_epoch, evaluate, train_one_epoch_training_time
...
def get_args_parser():
    ...
    parser = argparse.ArgumentParser('DST-DeiT training and evaluation script', add_help=False)

    parser.add_argument('--t_end', type=float, default=0.8, help='t end')
    parser.add_argument('--pruning_type', type=str, default='unstructure', help='The decay schedule for the pruning rate. Default: cosine. Choose from: cosine, linear.')  
    parser.add_argument('--token_selection', action='store_true')
    parser.add_argument('--train_eval', action='store_true')

    # required for structure pruning
    parser.add_argument('--atten_head', default=3, type=int, help='tiny:3, small:6, base:12')
    parser.add_argument('--atten_density', type=float, default=0.2, help='Dropout rate (default: 0.)')
    parser.add_argument('--other_density', type=float, default=0.2, help='Dropout rate (default: 0.)')
    parser.add_argument('--token_number', default=197, type=int)
...
def main(args):
    ...
    if not args.token_selection:
        print('unset token selection')
        delattr(model, 'gumbel')
        for name, m in model.named_modules():
            if hasattr(m, 'gumbel'):
                delattr(m, 'gumbel')

    if args.pruning_type == 'structure_new':
        print('enable score function for structure pruning')
        for name, m in model.named_modules():
            if hasattr(m, 'score'):
                m.score = True
        ...
        if args.output_dir:
            ...
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master({
                    ...
                    'args': args,
                    'mask': mask.masks,
                    'other_mask': mask.other_masks,
                    'atten_mask': mask.atten_masks,
                    'mask_step': mask.steps,
                    'fired_mask': mask.fired_masks
                }, checkpoint_path)
    ...
    layer_fired_weights, total_fired_weights = mask.fired_masks_update(args.pruning_type)
    ...
    print('* (RS) The final percentage of the total fired weights is:', total_fired_weights)
